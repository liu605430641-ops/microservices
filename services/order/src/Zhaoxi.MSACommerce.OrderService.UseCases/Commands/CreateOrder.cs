    ...
    ICapPublisher                      capPublisher) : ICommandHandler<CreateOrderCommand,Result<long>>
{
    ...
    {
        var order = new Order(idGen.CreateId()) { UserId = user.Id,Receiver = "张三",ReceiverAddress = "朝夕",PaymentType = request.Order.PaymentType };
            ...
            var orderCreatedEvent = new OrderCreatedEvent()
                                    {
                                        OrderId = order.Id,
                                        UserId  = user.Id,
                                        Skus = order.OrderDetails.Select(x => new OrderSku(x.SkuId,x.Quantity))
                                                    .ToList()
                                    };
